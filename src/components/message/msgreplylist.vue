<template>
    <el-table :data="filteredData" style="width: 100%">
      ...
      <el-table-column label="留言外键" prop="messageId" />
      <el-table-column label="留言者" prop="name" />
      <el-table-column label="邮箱" prop="email" />
      <el-table-column label="留言内容" prop="reply" />
  ...
  import { getReplyList,delteMsgReply } from "../../http/modules/message"
  ...
  const handleDelete = async (index: number, row) => {
    ...
    .then(async () => {
        const res = (await delteMsgReply(row.id))
  ...
  const load = async () => {
    ...
    const res = await getReplyList(currentPage.value, pageSize.value);
    ...
    if (search.value) {
      return data.value.filter((item) => {
        return (
          ...
          item.reply.toLowerCase().includes(search.value.toLowerCase())
        );
      });
    } else {
      return data.value;
